...
our @EXPORT_OK = qw(graph_line_image graph_map_image get_graphviz_supported);
...
sub graph_line_image {
    my ($map, $line_name, %args) = @_;
    $args{format} ||= 'png';
    $args{format} = lc($args{format});
    if ( exists $args{output_file} && !defined $args{output_file} ) {
      $args{output_file} = $map->name() . '_' . $line_name . '.' . $args{format};
      $args{output_file} =~ s/[^\w.]+/_/g;
    }
    ...
    my %neighbour;
    @neighbour{ map { $g->successors($_) } @line_v } = ();
    delete @neighbour{ @line_v };
    ...
    my $gvgraph = GraphViz2->from_graph($g);
    my $output;
    if ( lc($args{format}) eq 'gv' ) {
      if ( exists($args{output_file}) ) {
        open( my $outf, '>', $args{output_file} ) or croak "Cannot write output file '$args{output_file}': $!";
        print $outf $gvgraph->dot_input();
        close($outf);
      }
      $output = $gvgraph->dot_input();
    } else {
      $output = $gvgraph->run(%args)->dot_output();
    }
    return ( $output, $args{output_file} );
}
...
sub graph_map_image {
    ...
    $args{format} = lc($args{format});
    if ( exists $args{output_file} && !defined $args{output_file} ) {
      $args{output_file} = $map->name() . '.' . $args{format};
      $args{output_file} =~ s/[^\w.]+/_/g;
    }
    my $bgcolor = $map->bgcolor;
...
sub get_graphviz_supported {
    my ($what) = @_;
    my $dot_bin = which('dot');
...
sub _graph_line_label {
    ...
    return sprintf("%s Map: %s Line (Generated by Map::Tube::Plugin::Graph %s at %s)",
...
sub _graph_map_label {
    ...
    $map_name = '' unless defined $map_name;
    return sprintf("%s Map (Generated by Map::Tube::Plugin::Graph %s at %s)",
...
Copyright (C) 2015 - 2026 Mohammad S Anwar.
